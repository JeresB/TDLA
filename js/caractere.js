    ...
    db.get("personnages").value().forEach((perso, index) => {
        html += `
            <tr>
                ...
                <td><input class="form-control box-shadow-inset updateCaractere" type="text" data-index="${index}" data-champ="icone" value="${perso.icone}" /></td>
...
$(document).on('click', '#btnExportJson', function () {
    exportToJsonFile({
        persos: db.get("personnages").value(),
        tasks: db.get("taches").value(),
        times: db.get("horaires").value(),
...
function loaded(evt) {
    ...
    db.get("personnages").set(fileJSON.persos);
    db.get("taches").set(fileJSON.tasks);
    db.get("horaires").set(fileJSON.times);
